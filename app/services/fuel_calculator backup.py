...
class FuelCalculator:
    ...
    def distribute_fuel(self, received, generators, balances):
        ...
        # остаток выдаем по 5 л генераторам с наибольшей нехваткой,
        # не превышая потребность на сутки
        remainder = received - distributed
        while remainder >= self.FUEL_STEP:
            candidates = [g for g in generators if result[g["inventory"]] + self.FUEL_STEP <= needs[g["inventory"]] + self.FUEL_STEP]
            if not candidates:
                break
            target = max(candidates, key=lambda g: needs[g["inventory"]] - result[g["inventory"]])
    ...
    def calculate_decade(self, year, month, decade):
        ...
        start_date, end_date = self.get_decade_dates(
            year,
            month,
            decade
        )
